 ...
 */    class ItcSlider {
        ...
        #onSwipeEnd(e) {
            ...
            if (this.#config.loop) this.#autoplay();
        }
        #onTransitionStart() {
            if (this.#state.isBalancing) return;
        ...
        #onVisibilityChange() {
            if (document.visibilityState === "hidden") this.#autoplay("stop"); else if (document.visibilityState === "visible" && this.#config.loop) this.#autoplay();
        }
        #attachEvents() {
            this.#state.events = {
        ...
        #autoplay(action) {
            ...
            if (this.#state.intervalId === null) this.#state.intervalId = setInterval((() => {
                this.#state.direction = "next";
                this.#move();
        ...
        #updateClasses() {
            ...
            this.#state.activeItems.forEach(((item, index) => {
                if (item) this.#state.elListItem[index].classList.add(activeClass); else this.#state.elListItem[index].classList.remove(activeClass);
                const elListIndicators = this.#state.el.querySelectorAll(`.${this.#state.prefix}${this.constructor.#EL_INDICATOR}`);
                if (elListIndicators.length && item) elListIndicators[index].classList.add(`${this.#state.prefix}${this.constructor.#EL_INDICATOR_ACTIVE}`); else if (elListIndicators.length && !item) elListIndicators[index].classList.remove(`${this.#state.prefix}${this.constructor.#EL_INDICATOR_ACTIVE}`);
            }));
        }
        #move() {
            ...
            if (!this.#config.loop) {
                ...
                if (transform < -limit || transform > 0) return;
                if (this.#state.btnPrev) {
                    this.#state.btnPrev.classList.remove(this.#state.btnClassHide);
                    this.#state.btnNext.classList.remove(this.#state.btnClassHide);
                }
                if (this.#state.btnPrev && transform === -limit) this.#state.btnNext.classList.add(this.#state.btnClassHide); else if (this.#state.btnPrev && transform === 0) this.#state.btnPrev.classList.add(this.#state.btnClassHide);
            }
        ...
        #moveTo(index) {
            ...
            if (delta !== 0) {
                ...
                for (let i = 0; i < Math.abs(delta); i++) this.#move();
            }
        }
        #init() {
            this.#state.els = [];
            ...
            if (this.#config.loop) {
                ...
                this.#updateExProperties();
            } else if (this.#state.btnPrev) this.#state.btnPrev.classList.add(this.#state.btnClassHide);
            this.#updateClasses();
            this.#autoplay();
        }
        #reset() {
            ...
            if (widthItem === this.#state.width && countActiveEls === this.#state.countActiveItems) return;
            this.#autoplay("stop");
            this.#state.elItems.classList.add(transitionNoneClass);
